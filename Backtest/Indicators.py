...
def ADX(df, n, n_ADX):
    ...
    ADX = Series(ewma(abs(PosDI - NegDI) / (PosDI + NegDI), span=n_ADX, min_periods=n_ADX - 1),
...
def ULTOSC(df):
    ...
    while i < df.index[-1]:
        ...
        BP_l.append(BP)
        i = i + 1
    UltO = Series((4 * rolling_sum(Series(BP_l), 7) / rolling_sum(Series(TR_l), 7)) + (
    2 * rolling_sum(Series(BP_l), 14) / rolling_sum(Series(TR_l), 14)) + (
                  rolling_sum(Series(BP_l), 28) / rolling_sum(Series(TR_l), 28)), name='Ultimate_Osc')
    df = df.join(UltO)
    return df
